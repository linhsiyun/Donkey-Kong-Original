...
void OpeningScene::Update(float dt) {
    ...
    if (m_Phase == 0) {
        ...
        auto anim = std::dynamic_pointer_cast<Util::Animation>(m_DonkeyKong->GetDrawable());
        if (anim) {
            anim->SetCurrentFrame(13);
        }
        ...
        m_DonkeyKong->SetLocked(false); // 解鎖，讓大金剛恢復正常
        m_DonkeyKong->ResetToGame();    // 恢復正常丟木桶邏輯
    }
}